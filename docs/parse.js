...
class SolarData {
	...
	parse(raw, i) {
		if (is_valid_sep(raw, i + 12)) {
			let buffer = (raw.charCodeAt(i + 1) - SHIFT);
			buffer += (raw.charCodeAt(i + 2) - SHIFT) << 6;
			buffer += (raw.charCodeAt(i + 3) - SHIFT) << 12;
			buffer += (raw.charCodeAt(i + 4) - SHIFT) << 18;
			buffer += (raw.charCodeAt(i + 5) - SHIFT) << 24;
			this.date.push((buffer & 0xfffffff) + 1690000000);
			buffer >>= 28;

			buffer += (raw.charCodeAt(i + 6) - SHIFT) << 2;
			buffer += (raw.charCodeAt(i + 7) - SHIFT) << 8;
			buffer += (raw.charCodeAt(i + 8) - SHIFT) << 14;
			this.power.push((buffer & 0x7fff) / 10);
			buffer >>= 15;

			buffer += (raw.charCodeAt(i + 9) - SHIFT) << 5;
			this.frequency.push(((buffer & 0x7ff) + 4000) / 100);

			buffer = (raw.charCodeAt(i + 10) - SHIFT);
			buffer += (raw.charCodeAt(i + 11) - SHIFT) << 6;
...
class DiagnosticData {
	...
	parse(raw, i) {
		if (is_valid_sep(raw, i + 8)) {
			...
			buffer += (raw.charCodeAt(i + 5) - SHIFT) << 2;
